...
Install to first deny receipt, and then verify that receipt yourself. Copy this
...
With the starter policy, a secret-bearing outbound POST is denied before `fetch`
...
[`examples/quickstart`](examples/quickstart) for the file this snippet came from.
...
<!-- vectors:begin -- generated by `npm run gen:vectors`, do not edit by hand -->

- 26 of 26 adversarial vectors denied under the starter policy `starter-1`,
  reproducible in CI. Every vector, and the reason the engine returned for it, is
  in [`docs/VECTORS.md`](docs/VECTORS.md).
