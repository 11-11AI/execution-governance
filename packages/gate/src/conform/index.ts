...
/** The receipt minus its sig field: the object a signature is computed over. */
function withoutSig(r: Record<string, unknown>): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const k of Object.keys(r)) if (k !== "sig") out[k] = r[k];
  return out;
}
...
function signerAmong(
  ...
  let msg: Buffer;
  try {
    msg = sha3buf(canonicalJson(unsigned));
  } catch {
    return undefined;
  }
...
export function checkConformance(
  ...
  lines.forEach((raw, i) => {
    ...
    try {
      ...
      if (r === null || typeof r !== "object" || Array.isArray(r)) {
        add({
          rule: "R01",
          title: "each line is a JSON object",
          status: "fail",
          line,
          expected: "a JSON object",
          actual: Array.isArray(r) ? "an array" : String(r),
  ...
  for (const { line, r } of good) {
    ...
    for (const f of [...REQUIRED, ...OPTIONAL]) {
      if (f in r && typeof r[f] !== "string") {
        ...
        add({
          rule: "R03",
          title: "every field is a string",
          status: "fail",
          line,
          expected: `${f} is a string`,
          actual: `${f} is ${typeof r[f]}`,
          reference: "RECEIPTS.md § Fields",
        });
      }
    }
  ...
  const scalar: Array<[string, string, (r: Record<string, unknown>) => boolean, string]> = [
    ...
    [
      ...
      (r) => HEX128.test(String(r.argsHash)),
    ...
    for (const { line, r } of good) {
      ...
      if (!ok(r)) {
        ...
        add({
          ...
          line,
          expected,
          actual: JSON.stringify(r[field[rule]!]),
          reference: "RECEIPTS.md § Fields",
        });
      }
    }
  ...
  for (const { line, r } of good) {
    ...
    if (!key) {
      ...
      if (signer) {
        ...
        add({
          rule: "R09",
          title: "kid is the sha3-512 fingerprint of the public key",
          status: "fail",
          line,
          expected: kidFor(signer),
          actual: kid,
          reference: "RECEIPTS.md § Signature",
          detail:
            "this receipt's signature verifies under a supplied key, so the key is right and the kid is wrong. kid is the first 16 hex chars of sha3-512 over the RAW 32 key bytes, not over their base64url text",
        });
      }
      continue;
    }
  ...
  for (const { line, r } of good) {
    try {
      ...
      add({
        rule: "R10",
        title: "the receipt has a canonical form",
        status: "fail",
        line,
  ...
  const RAW_SIG_HINTS: Array<[string, (r: Record<string, unknown>, raw: string) => Buffer]> = [
    ...
    [
      ...
      (r) => sha3buf(canonicalJson({ ...withoutSig(r), sig: "" })),
    ],
    [
      "signed with sig present as null, rather than the key absent",
      (r) => sha3buf(canonicalJson({ ...withoutSig(r), sig: null })),
    ],
    ["signed the raw line as written, rather than its canonical form", (_r, raw) => sha3buf(raw)],
    [
      "signed the canonical bytes directly, rather than their sha3-512 digest",
      (r) => Buffer.from(canonicalJson(withoutSig(r)), "utf8"),
    ],
    [
      "signed JSON.stringify output in insertion order, rather than the canonical form",
      (r) => sha3buf(JSON.stringify(withoutSig(r))),
    ],
  ];
  ...
  if (keys.size === 0) {
    add({
      rule: "R11",
      title: "sig is Ed25519 over sha3-512 of the canonical receipt without sig",
    ...
    for (const { line, raw, r } of good) {
      ...
      if (!key) {
        ...
        add({
          ...
          detail:
            "this receipt was signed by a key that was not offered for checking. If the file spans a key rotation, supply every public key with a repeated --key. Until then this line is unverified, which is not the same as valid.",
        });
        continue;
      }
      ...
      let ok = false;
      try {
        ok = edVerify(null, sha3buf(canonicalJson(unsigned)), pub, sigBytes);
      } catch {
        ok = false;
      }
      if (ok) continue;
      ...
      for (const [why, preimage] of RAW_SIG_HINTS) {
        try {
          if (edVerify(null, preimage(r, raw), pub, sigBytes)) {
            detail = `the signature verifies over a different preimage: ${why}`;
            break;
          }
        } catch {
          /* probe only */
      ...
      add({
        ...
        actual: detail
          ? "verified over a different preimage, see detail"
          : "did not verify under any known preimage",
        reference: "RECEIPTS.md § Signature",
        detail:
          detail ??
          "the receipt may have been edited after signing, or signed by a different key than its kid names",
      });
    }
    if (!sigBad) {
      add({
  ...
  if (good.length > 0) {
    ...
    if (got !== GENESIS) {
      add({
        rule: "R12",
        title: 'the first receipt chains to the literal string "genesis"',
        status: "fail",
        line: first.line,
        expected: `"${GENESIS}"`,
        actual: JSON.stringify(first.r.prevReceiptHash),
        reference: "RECEIPTS.md § Chain",
        detail:
          "a hash of the empty string, 128 zeros, or an empty value are the usual wrong answers here",
      });
    } else {
      add({
        rule: "R12",
  ...
  for (let i = 1; i < good.length; i++) {
    ...
    let expected: string;
    try {
      expected = sha3hex(canonicalJson(prev.r));
    } catch {
      continue;
    } // R10 owns that failure
    const got = String(cur.r.prevReceiptHash);
    if (got !== expected) {
      chainBad = true;
      let hint: string | undefined;
      try {
        if (got === sha3hex(canonicalJson(withoutSig(prev.r)))) {
          hint =
            "this is sha3-512 of the previous receipt WITHOUT its sig. The chain digest covers the full previous receipt, signature included.";
        }
      } catch {
        /* ignore */
      }
      add({
        rule: "R13",
        title: "prevReceiptHash is sha3-512 of the canonical previous receipt, including its sig",
        status: "fail",
        line: cur.line,
        expected,
        actual: got,
  ...
  for (const f of findings) {
    ...
    if (f.status === "fail" || cur === undefined)
      byRule.set(f.rule, f.status === "fail" ? "fail" : f.status);
  }
